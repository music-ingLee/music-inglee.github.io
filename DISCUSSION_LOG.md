...
- **002 본문 평이화 + 브랜드명 표시 (운영자 지적 3건)** —
  - **"미래에도 과거에도 저마다 자리가 있다"류 삭제.** 추상 명제로 문을 열어 무슨 앱인지 모른 채 읽게 만들었다. "**맨날**은 오늘 할 일을 적는 앱이다. 열면 오늘 날짜가 먼저 뜨고, 곧바로 타이핑해 적는다"로 시작해 **무엇인지 → 어떻게 남는지 → 왜 오늘인지** 순으로 재배열. 현재 방점은 명제가 아니라 사실로 진술한다("달력에서 붉게 칠해지는 칸은 오늘 하나뿐이다").
  - **`.brand` 클래스 신설.** 「맨날」은 '매일'이라는 평범한 부사여서 한국어 문장 안에서 브랜드명인지 부사인지 구분이 안 된다(운영자 지적). 본문에서 따옴표를 쓰면 지저분하므로 **잉크색 700 웨이트**로만 표시 — 회색 본문 속 진한 한 단어가 고유명사로 읽힌다. 라틴 문자인 Thinket은 이 문제가 없어 적용하지 않음.
  - **"도형이 상태를, 채움이 증거의 등급을 맡는다" 폐기.** 내부 문서(`DESIGN_PHILOSOPHY.md` 4절) 용어를 그대로 옮긴 건데 독자에겐 아무 뜻이 아니다. 두 축을 각각 **무엇을 가리키는지 풀어서**: 도형 → "다 해낸 날은 원, 남긴 일이 있는 날은 삼각, 오늘과 아직 오지 않은 날은 사각", 채움 → "같은 도형이라도 **언제 적었는지**에 따라 진하기가 다르다. 그날 그 자리에서 적은 것은 진하게, 나중에 되살려 적은 것은 옅게". '증거의 등급' 같은 말은 한 개도 남기지 않았다.
  - 교훈: 내부 설계 문서의 압축된 용어는 그대로 옮기면 안 된다. 문서는 이미 합의된 사람들끼리 쓰는 말이고, 사이트는 처음 보는 사람이 읽는 말이다. 캐시버전 v51→v52.
