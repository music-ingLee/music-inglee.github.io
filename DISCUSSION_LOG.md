...
- **About 캡 완전 철회** — 48자(`--measure-wide`, 951px)로 줄여도 본문 칸이 1084px이라 **사진 우변에서 133px 못 미쳤다**(운영자 지적: 사진 우측 선까지 뻗길 기대). 사진은 우측 float라 그 오른쪽 변이 곧 칸의 오른쪽 변이므로, **캡이 있는 한 절대 닿지 않는다.** 토큰을 지우고 `.lede`를 무제한으로 되돌려 원래 상태 복원 — 실측 결과 마지막 문단 우변과 사진 우변의 차 **0px**, 사진 옆 39.4자 / 아래 54.4자. 사이트 유일한 measure 예외이며 DESIGN.md에 "일부러 치르는 값"으로 명시. 캐시버전 v50→v51.
- **002 도판 교체: 앱 스크린샷 → maennal.com 히어로의 달력** — 운영자 지적("사진자료가 별로 안 예뻐, 히어로 쪽이 알록달록하고 예쁘다"). 기존은 운영자 실데이터 7월이라 **원·삼각 두 도형밖에 안 나왔다**(과거 달이라 붉은 사각이 없고, 사후 기입도 없고, 기간 일정도 라벨 때문에 잘라냈다). 랜딩 히어로의 예시 8월은 문법이 전부 등장한다: 네이비 원 · 앰버 삼각 · **감광 삼각(10일=사후 기입)** · **붉은 사각(28일=오늘)** · **속 빈 사각(29일=미래)** · 검정/빨강/파랑 **띠(여러 날 일정)**. maennal.com을 2x로 띄워 `.month-meta` 상단부터 마지막 `.cal-day` 하단까지 클립(1442×1230, 비 1.172) — **기존 도판 비(1.19)와 거의 같아 레이아웃은 그대로**(판 351→346px). 캡션에 "사각 오늘"을 더했다(이제 실제로 보이므로). 한 줄 유지 확인. 실데이터가 아니라 **랜딩의 예시 월**이라는 점은 캡션이 개인 기록이라 주장하지 않게 중립적으로 뒀다.
- **002 본문 전면 재작성 — 방점은 과거가 아니라 현재 (운영자 교정)** — 내가 쓴 "미래 관리 앱들의 반대편에 선 과거 기록 앱"은 **대립 구도**였는데, 실제 논지는 **삼분 구조**다: 과거도 미래도 저마다 자리가 있고, 그럼에도 살 수 있는 건 오늘뿐이라 방점이 현재에 찍힌다. 운영자 판정 "왜곡에 가깝다" — 동의. `DESIGN_PHILOSOPHY.md`를 안 읽고 마케팅 대립각으로 짠 게 원인이었고, 근거는 문서에 이미 다 있었다: 「솔리드 = 오늘(**지금 여기**)」 · `--danger` 빨강은 "**현재-시각 선**"에만 · 「미래는 판정을 받지 않는다」(속 빈 사각 + **hover하면 채워지는 '그날을 미리 살아보는' 프리뷰**) · 과거는 읽기 전용이라 「되살리기」 1회 경유 · **앱 아이콘 축약형 = "붉은 사각(오늘) 하나를 중심에 둔 최소 구성"**. 즉 과거를 굳히고 미래를 유예하는 장치가 **둘 다 손을 오늘에 두게 하는 설계**였다. 새 1문단은 세 시제에 각각 자리를 주고 "붉은 사각은 오늘에게만" → "가장 크게 울리는 자리는 언제나 지금"으로 방점을 세우고, 2문단은 불변·유예를 **같은 목적의 두 장치**로 묶어 초상화로 닫는다. 마침 교체한 히어로 도판에 붉은 사각(28일)이 유일하게 강렬해서 글과 그림이 같은 말을 한다.
- **칸딘스키·바우하우스 귀속 삭제 (운영자 판단: 독창성을 흐린다)** — 사이트 문안에서 뺐다. 실제 독창성은 색-도형 대응이 아니라 **매핑**(도형=상태 · 채움=증거의 등급)이고, 그건 빌려온 것이 아니다. 영향 관계 자체는 `daily-todo/DESIGN_PHILOSOPHY.md` 4절이 정직하게 기록하고 있으므로 사실이 지워진 게 아니라 **포트폴리오에서 계보를 앞세우지 않기로** 한 것. 필요하면 003의 「작가의 말」처럼 조용한 gloss 툴팁으로 되살릴 수 있다.
- 스택 문장은 `work__spec`(macOS 앱 · 웹 PWA · React, Electron)과 중복이라 걷고, 고유한 iCloud 양방향 미러링만 짧은 3문단으로. 바닥 차이 +18 → **−19**(글이 판보다 19px 김)로 오히려 더 붙었다.
- **CV에도 같은 교정** — Projects의 맨날 줄이 "records the day rather than plans it; the month becomes a **Bauhaus** composition"으로, 과거 편향 서술과 귀속이 둘 다 남아 있었다(index.html만 고치고 놓칠 뻔). "weighted on the present: the past is frozen, the future left unjudged, only today open to write"로 교체. cv.pdf 재생성, 한 장 유지 확인. (index.html에 남은 em dash 2곳은 `<title>`과 뮤지킹 `(Neo)Soul —` — 2026-07-01에 운영자 확인 대기로 남긴 그 둘이라 이번에도 손대지 않음.)
