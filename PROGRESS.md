...
- **MoMA 라벨화 (2026-06-13, 사진 레퍼런스: 미술관 벽 네임플레이트)**: 사용자 결정 — ① 레이아웃=**노트형**(본문 statement는 노출 유지, 호버는 작은 라벨 노트만 + 타이포만 미술관화) ② **캡션 스펙 라인 도입**(`.work__spec`, 매체·재료·규격) ③ **QR 없음**.
  ...
  - styles `?v=29→30`.
- **재균형 + 탈현학 (2026-06-13)**: 사용자 피드백 — 첫 문단("무엇이 인간을 인간이게 하는가… 표현의 극단… 불완전성이 쌓여 한 연주자의 색") **과하게 현학적 + 주객전도**(작품 핵심=제목 그대로 **공감각**인데 인간성 탐구 preamble이 본문을 잡아먹음).
  - 해결: 인간다움 탐구 preamble **전면 삭제**(이미 호버 "작가의 말"에 있음). 본문 호흡 = **왜(인간성 철학=호버 한 구절) → 방법(확률·무선화 로직, 조성·4악기 틀) → 공감각 시각화 → 미적 쾌감**. 이미지 레퍼런스 호흡 따름("미적 쾌감이 솟는다"·"구현했다" 어미 채용, "창발" 같은 jargon 후퇴).
  - 미적 쾌감 close를 **2단락(공감각)** 끝으로 이동 → 공감각이 본문의 도착점=중심. 1단락은 의도+방법으로 짧게.
- **연도 통합 (2026-06-13)**: 우측 `work__year` div 3개 제거 + `.work` 그리드 `64px 1fr 90px`→`64px 1fr`. 연도는 `.work__spec` 라인 끝으로(`… · 2026`). 라벨식 한 줄 캡션 완성. styles `?v=30→31`.
- **공감각 중심 + 담백화 (2026-06-13)**: 사용자 재지적 — ① "모든 것을 무선화하면 혼돈이어서…제한했다"=**변명조** → 삭제, "조성이라는 최소한의 틀 속에서 서로 다른 네 악기의 즉흥 연주를 시도했다"로 담백하게 ② "구현했다"→**"구현하고자 했다"**(단정 회피) ③ 공감각 단락이 너무 짧음 → **이미지 레퍼런스 맥락 복원**: 음색→색 lcd 매핑 / 각 성부 소리 텍스처의 빗방울 인상 → 공감각을 파동으로 치환 / 빗방울 로직 구현하자 lcd에 실제 비 내리는 장면. 공감각이 본문의 主. ⚠️ "lcd에 비 내리는 장면"은 과거 '유치'로 삭제했으나 **사용자가 명시적으로 이 맥락 포함 요구**해 복원(사용자=최종 권위). HTML만 변경(버전 bump 없음).
