...
- **studyForest 카피 최종 (2026-06-13)**: 도입을 안전한 일반 명제로 시작 — "탐구는 본래 선이 아니라 나무 모양으로 자란다"
  (모든 사고가 항상 분기한다는 절대 주장은 회피, "으레/본래"로 완화) → LLM 선형 스크롤이 그 분기를 못 담음으로 연결.
  **em dash 전면 제거**(사용자 선호), 쉼표 동격·문장 분할로 재조직. "맥락이 사용자의 통제 아래로 들어온다"(논문의 user-controllable context 근거).

## Works 003 — Synaesthesia 카피 (2026-06-13, MoMA 톤 재작성)
- 기존: "조성을 고정한 채 확률 로직으로…" 단문 → **미술관 작품 라벨 온도**로 재조직(사용자 의도 부연 받아 Claude가 조직).
- 담은 내용: ① note 등장 타이밍에 확률 로직 + duration·velocity 무작위 = '즉흥성'을 알고리즘으로 구현
  ② 악기 4대, 음색마다 떠오르는 공감각 색을 `lcd`로 실제 매핑·시각화 ③ 성부 텍스처가 빗방울 감각 → 무작위 소리의 빗방울 로직 짜니 화면에도 비 내림
  ④ 완전 무작위가 아닌 "최소한의 틀 안의 즉흥"에서 미적 쾌감. em dash 없음.
